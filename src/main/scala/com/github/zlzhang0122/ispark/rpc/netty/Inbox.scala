...
import com.github.zlzhang0122.ispark.rpc.{RpcAddress, RpcEndpoint, ThreadSafeRpcEndpoint}
          ...
          case RpcMessage(_sender, content, context) =>
            try {
              endpoint.receiveAndReply(context).applyOrElse[Any, Unit](content, { msg =>
                throw new ISparkRpcException(s"Unsupported message $message from ${_sender}")
              })
            } catch {
              case e: Throwable =>
    ...
    try action catch {
      case NonFatal(e) =>
        try endpoint.onError(e) catch {
          case NonFatal(ee) =>
            if (stopped) {
              log.debug("Ignoring error", ee)
            } else {
              log.error("Ignoring error", ee)
            }
        }
    }
  }
