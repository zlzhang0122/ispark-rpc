...
import com.github.zlzhang0122.ispark.rpc.RpcAddress
...
private[netty] case class OneWayOutboxMessage(content: ByteBuffer) extends OutboxMessage {
  ...
  override def onFailure(e: Throwable): Unit = {
    e match {
      case e1: RpcEnvStoppedException => log.debug(e1.getMessage)
...
private[netty] class Outbox(nettyEnv: NettyRpcEnv, val address: RpcAddress) {
  ...
  def send(message: OutboxMessage): Unit = {
    ...
    if (dropped) {
      message.onFailure(new ISparkRpcException("Message is dropped because Outbox is stopped"))
    } else {
      drainOutbox()
    }
  }
