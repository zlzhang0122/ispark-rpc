...
package com.github.zlzhang0122.ispark.rpc.netty
...
import com.github.zlzhang0122.ispark.rpc.{RpcAddress, RpcEndpoint, RpcEndpointAddress, RpcEndpointNotFoundException, RpcEndpointRef, RpcEnv, RpcEnvConfig, RpcEnvFactory, RpcTimeout}
import com.github.zlzhang0122.ispark.serailizer.{JavaSerializer, JavaSerializerInstance}
import com.github.zlzhang0122.ispark.util.{ThreadUtils, Utils}
...
class NettyRpcEnv(
                   val conf: ISparkRpcConf,
                   javaSerializerInstance: JavaSerializerInstance,
                   host: String,
                   numUsableCores: Int) extends RpcEnv(conf) {

  private val log = LoggerFactory.getLogger(classOf[NettyRpcEnv])

  private[netty] val transportConf = ISparkRpcTransportConf.fromSparkConf(
...
private[rpc] class NettyRpcEnvFactory extends RpcEnvFactory {
  ...
  def create(config: RpcEnvConfig): RpcEnv = {
    ...
    val nettyEnv =
      new NettyRpcEnv(conf, javaSerializerInstance, config.bindAddress, config.numUsableCores)
...
private[netty] class NettyRpcEndpointRef(
                                          @transient private val conf: ISparkRpcConf,
