...
import com.github.zlzhang0122.ispark.rpc.{RpcEndpoint, RpcEndpointAddress, RpcEndpointRef}
import com.github.zlzhang0122.ispark.util.ThreadUtils
    ...
    while (iter.hasNext) {
      val name = iter.next
        postMessage(name, message, (e) => { e match {
          case e: RpcEnvStoppedException => log.warn(s"Message $message dropped. ${e.getMessage}")
          case e: Throwable => log.warn(s"Message $message dropped. ${e.getMessage}")
        }}
      )}
  }
  ...
  private def postMessage(
    ...
    val error = synchronized {
      ...
      if (stopped) {
        ...
        Some(new ISparkRpcException(s"Could not find $endpointName."))
      } else {
