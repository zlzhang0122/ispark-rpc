...
import com.github.zlzhang0122.ispark.util.Utils
...
class SimpleRpcTest extends BaseRpcTest {
  ...
  "EchoEndpoint" should "fail when not endpoint found" in {
    ...
    val thrown = the[ISparkRpcException] thrownBy clientCall(EchoEndpoint.ENDPOINT_NAME)(runBlock, assertBlock)
  ...
  "EchoEndpoint" should "fail due to remote port not correct" in {
    ...
    val thrown = the[ISparkRpcException] thrownBy clientCall(EchoEndpoint.ENDPOINT_NAME)(runBlock, assertBlock, port = 9999)
    thrown.getMessage should equal("Exception thrown in awaitResult")
  ...
  "EchoEndpoint" should "fail due to remote address not correct" in {
    ...
    val thrown = the[ISparkRpcException] thrownBy clientCall(EchoEndpoint.ENDPOINT_NAME)(runBlock, assertBlock, rpcConf = rpcConf, host = "xxxmm")
  ...
  "EchoEndpoint" should "run locally successfully" in {
    ...
    val config = RpcEnvServerConfig(new ISparkRpcConf(), "hello-server", _host, _port.get())
